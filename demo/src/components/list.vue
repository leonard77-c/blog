...
export default {
  ...
  created() {
    this.keywords.title = this.$route.query.keyword;
    this.tagUid = this.$route.query.tagUid;
    this.sortUid.name = this.$route.query.sortUid;
    this.author.username = this.$route.query.author;
  ...
  methods: {
    ...
    search: function() {
      ...
      let that = this;
      //that.loading = true;
      if (this.keywords.title != undefined) {
      ...
      }else if(this.author.username!=undefined) {
      ...
      }else if(this.sortUid.name!=undefined) {
        const url = 'http://localhost:9090/api/cate/article/search';
          const data = this.sortUid;
          axios.post(url, data).then(response => {
            console.log(this.sortUid.name);
            that.convertSearchData(that, response)
          });
      }
    }
...
.container { width: 1600px;height: 1500px; margin: 20px auto; background-color:gainsboro;position:relative;}
a{
