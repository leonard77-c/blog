<template>
  <div class="cloud" v-if="hotTagData.length > 0">
    ...
    <ul>
      <a v-for="item in hotTagData" :key="item.id" href="javascript:void(0);" @click="goToList(item.name)">{{item.name}}</a>
    </ul>
  </div>
</template>
...
export default {
  ...
  created() {
            const url = 'http://localhost:9090/api/return_cate';
            //const data = this.blogOid;
            axios.post(url).then(response => {
            //console.log(this.keywords.title+'nnn');
           
                this.hotTagData = response.data.cate;
...
<style>
.cloud { width:300px;clear: both; overflow: hidden; background: #fff; margin-bottom: 20px }
